...
Another thing that must be taken in mind is that inter-series comparisons must be understood correctly. You cannot compare an F1 car to a gas efficient consumer car. Each is designed for a specific purpose and this must be kept in mind. You use one when you want to go FAST, and the other if you like your rides to be comfortable. This also applies to inter-generational comparisons, it's expected that the more recent version of a CPU _should_ be better. 
